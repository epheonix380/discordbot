...
async def handle_nsfw(message: discord.Message):
    ...
    for thing in message.attachments:
        try:
            ...
            with open(f"temp/SPOILER_{i}_{thing.filename}", "wb") as handler:
                handler.write(img_data)
                obj = nude_detector.detect(f"temp/SPOILER_{i}_{thing.filename}")
                print(obj)
                if check_conditions(obj, isDom=str(message.author.id) == "226315767564599297"):
                    nsfw_count = nsfw_count + 1
                    trigger = trigger and False
                    files.append(discord.File(f"temp/SPOILER_{i}_{thing.filename}"))
                else:
    ...
    for match in arr:
        try:
            if match.group(0) is not None:
                ...
                with open(f"temp/SPOILER_{i}.png", "wb") as handler:
                    handler.write(img_data)
                    obj = nude_detector.detect(f"temp/SPOILER_{i}.png")
                    print(obj)
                    ...
                    if check_conditions(obj, isDom=str(message.author.id) == "226315767564599297"):
                        ...
                        containsEmbeds = True
                        trigger = trigger and False
                        files.append(discord.File(f"temp/SPOILER_{i}.png"))
                    else:
